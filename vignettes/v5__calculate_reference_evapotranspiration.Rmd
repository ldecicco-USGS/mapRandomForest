...
df <- df %>% mutate(yyyymmdd=lubridate::ymd(paste(date,"15",sep="-"))) %>% mutate(year=lubridate::year(yyyymmdd)) %>%  mutate(days_in_month=lubridate::days_in_month(yyyymmdd)) %>% mutate(day_of_year=as.integer(yyyymmdd - lubridate::ymd(paste(year-1,"-12-31"))))
...
df <- df %>% mutate(Hargreaves_ET0=hargreavesET0(tmin, tmax, day_of_year, days_in_month, precip, lat))
