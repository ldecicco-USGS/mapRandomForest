...
# Initialize environment and read shapefile
...
Latitude and longitude associated with the USGS gageing staion should be availavle from NWIS. We'll calculate the latitude and longitude of the centroid to use in the event that the NWIS values are missing.

```{r}
# calculate centroid for projected dataset
centroids_sf <- sf::st_centroid(sites_sf)

# transform to unprojected (geographic) crs
...
# write out 
#readr::write_rds(centroid_coordinates_df,"centroid_coordinates.rds", compress="gz")

head(centroid_coordinates_df)
```
