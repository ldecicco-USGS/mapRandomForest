...
#testFlowDat_file <- system.file("extdata","testFlowDat.csv.gz", package = "mapRandomForest")
testFlowDat_file <- "testFlowDat.csv.gz"
...
## Clean up original random forest input dataset
...
#Run model
rfModTrained <- randomForest::randomForest(x=full,             # dataframe of predictor variables
                                           y=fullY,            # response vector
                                           ntree = 500,        # number of trees to grow
                                           mtry = 15,          # number of variables to select at random at each split
                                           do.trace=TRUE, 
                                           keep.forest=TRUE)

# ctrl <- caret::traincontrol(method = "oob",           # resampling method: 'Out Of Bag'
#                             number = 10,              # number of k-folds or resampling iterations
#                             allowParallel = TRUE,     # use parallel computation if available?
#                             verboseIter = TRUE)       # print training log?
# 
# train(full, fullY, method = "parRF", na.action = na.omit, 
#                  trControl = ctrl, tuneGrid = rfGrid, do.trace=TRUE))
...
ungagedDat_filename <- system.file("extdata","testUngagedDat.csv.gz", package = "mapRandomForest")
...
gagedDat_filename <- system.file("extdata","testGagedDat.csv.gz", package = "mapRandomForest")
...
# bigRiverDat <- readr::read_csv("F:/swMERAS/bigRivers/newBaseflow.csv.gz")
# 
...
# 
# write.csv.gz(finalFlowDat, "totalFlowDat.csv.gz", row.names = FALSE)
# 
